resource "aws_backup_plan" "backup_plan" {
  ...
  tags = {
    Name                   = "czr-raas-${var.region_code}-${var.environment}-backup-plan"
...
resource "aws_backup_selection" "existing_backup_resource_selection" {
  iam_role_arn = data.aws_iam_role.iam_role.arn
  name         = var.existing_backup_resource
  plan_id      = aws_backup_plan.backup_plan.id

 resources = [
     "arn:aws:ec2:${var.region}:${var.account}:instance/i-023d2d83aa88c9d6e",
      "arn:aws:ec2:${var.region}:${var.account}:instance/i-0db73d438cbcf5493",
       "arn:aws:ec2:${var.region}:${var.account}:instance/i-06f5ee44631504e3c"
  ]
}
